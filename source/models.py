...
def get_available_models() -> dict[str, type[lightning.LightningModule]]:
    """Discovers and returns all LightningModule subclasses in this file."""
    models = {}
    for name, obj in inspect.getmembers(sys.modules[__name__], inspect.isclass):
        if issubclass(obj, lightning.LightningModule) and obj != lightning.LightningModule:
            models[name] = obj
    return models
...
class AutoencoderBase(BaseModel):
    ...
    def on_test_end(self) -> None:
        ...
        self.figure.savefig(f'{self.save_dir}/report_{self.training_type}_{time.strftime("%Y%m%d-%H%M%S")}.png')
